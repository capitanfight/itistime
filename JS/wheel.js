...
export class Wheel {
    ...
    set(slices_loadout, slices, radius, colors, t_spin, speed, size_pointer) {
        this.t_spin = t_spin
        this.speed = speed
    ...
    createSlices() {
        // console.log("Log: Creating slices.")
        canvas.clear()

        let ang = (Math.PI * 2) / this.n_slices

        for (let i = 0; i < this.n_slices; i++) {
            // console.log("Log: Creating slices class for id: " + i + ".")
    ...
    spin = () => {
        ...
        // this.v = Math.random() * this.t_spin
    ...
    update = (t) => {
        ...
        // this.t += (t / 10000)
        // this.a = -(this.t_spin / (this.t + 1)) + 1
        // this.v += this.a
        // this.movedAngle += this.v
        // this.v -= .05 / (this.t_spin / this.v)
        // if (this.v < this.min_vel) {
        //     this.v = 0
        // }
        ...
        this.slices_array.forEach((slice, id) => {
            if (collide(this.pointer.p1, slice.p1, slice.p2)) {
                ...
                if (slice.isBonus) {
                    ...
                    switch (slice.name) {
                        case "coinFlip":
                            const { COIN_FLIP_MULTIPLIER, PSEUDOWHEEL_T, PSEUDOWHEEL_G, COIN_SPEED, COIN_T, WAIT } = this.bonus_setting.coin_flip
...
class MainWheel extends Wheel {
    constructor(slices_loadout, slices, radius, colors, t_spin, min_vel, size_pointer, bonus_setting) {
        super(slices_loadout, slices, radius, colors, t_spin, min_vel, size_pointer)
